package scenesC
{
	...
	public class IntheWater extends Sprite
	{
		...
		private function setDialog():void{
			diashift = true;
			diadown = false;
			speaking = new Dialogue('iw', 0, gameBG.height, gameBG.width, gameBG.height * 0.2);
			if(!preS){
				speaking.count = 1;
			}
			addChild(speaking);
		...
		public function close(e:Event):void{
			...
			if(e.type == "itemClick5"){
				dispatchEvent(new Event(IntheWater.PAGE_CLOSE1));
			}else{
				dispatchEvent(new Event(IntheWater.PAGE_CLOSE2));
			}
		}
