package scenesC
{
	...
	public class FrontDeckFL extends Sprite
	{
		...
		private function init():void{
			gameBG = new Loader();
			gameBG.load(new URLRequest("image/scene/InTheWater.jpg"));
		...
		private function setGBG(e:Event):void{
			...
			soundFactory = new Sound(new URLRequest("materials/ocean.mp3"));
		...
		private function setDialog():void{
			diashift = true;
			diadown = false;
			speaking = new Dialogue('fdfl', 0, gameBG.height, gameBG.width, gameBG.height * 0.2);
			addChild(speaking);
		...
		public function close(e:Event):void{
			...
			if(e.type == "itemClick5"){
				dispatchEvent(new Event(FrontDeckFL.PAGE_CLOSE1));
			}else{
				dispatchEvent(new Event(FrontDeckFL.PAGE_CLOSE2));
			}
		}
