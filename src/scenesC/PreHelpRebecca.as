package scenesC
{
	...
	public class PreHelpRebecca extends Sprite
	{
		...
		public function PreHelpRebecca()
		{
			super();
			init();
		}
		...
		private function init():void{
			gameBG = new Loader();
			gameBG.load(new URLRequest("image/scene/Rebecca_Waiter_SideDeckBoats.jpg"));
		...
		private function setDialog():void{
			...
			diadown = false;
			if(preS){
				speaking = new Dialogue('phrpr', 0, gameBG.height, gameBG.width, gameBG.height * 0.2);
			}else{
				speaking = new Dialogue('phrd', 0, gameBG.height, gameBG.width, gameBG.height * 0.2);
			}
			addChild(speaking);
		...
		private function close():void{
			...
			dispatchEvent(new Event(PreHelpRebecca.PAGE_CLOSE));
